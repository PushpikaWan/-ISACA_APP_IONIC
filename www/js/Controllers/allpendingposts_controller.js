...
.controller('AllPendingPostsController', ['$scope', '$http', '$state','$rootScope','$ionicPopup',
     function($scope, $http, $state,$rootScope,$ionicPopup) {
     
    $scope.image_url= 'http://'+$rootScope.url+'/my_project/img/tweet_img/';
    var link = 'http://'+$rootScope.url+'/my_project/allpending_posts.php';
                $http.post(link, {
    ...
    $scope.accept_post = function (tweet_id) {
        var link = 'http://'+$rootScope.url+'/my_project/accept_post.php';
                $http.post(link, {
                                userID : $rootScope.userID,
                                tweetID :tweet_id
                                }).then(function (res){
    ...
    $scope.showAccept = function(twt_id) {
        var confirmPopup = $ionicPopup.confirm({
        title: 'Accept post',
        template: 'Are you sure you want to accept this post?'
        });
        confirmPopup.then(function(res) {
        if(res) {
            $scope.accept_post(twt_id);
    ...
    $scope.delete_post = function (tweet_id) {
        var link = 'http://'+$rootScope.url+'/my_project/delete_post.php';
            ...
            // A confirm delete dialog
    $scope.showDelete = function(twt_id) {
        var confirmPopup = $ionicPopup.confirm({
        title: 'Delete post',
